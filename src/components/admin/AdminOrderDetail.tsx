    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Button className="mt-4 bg-[#0C831F] text-white" onClick={() => navigate('admin-orders')}>{t('orderDetail.backToOrders')}</Button>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-3xl mx-auto px-4 py-4">
        ...
        <div className="bg-gradient-to-r from-[#0C831F] to-[#0fa828] rounded-2xl p-5 text-white mb-4">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-green-200 text-sm">{t('orderDetail.order')}</p>
        ...
        <Card className="mb-4">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-bold text-gray-900">{t('orderDetail.orderStatus')}</h3>
              <span className={`px-3 py-1 rounded-full text-xs font-bold ${statusColor[order.orderStatus]}`}>
                {ts(statusTranslationKeys[order.orderStatus] || 'status.pending')}
              </span>
            </div>
            {/* Status Flow */}
              ...
              {['Pending', 'Accepted', 'Packing', 'Ready for Pickup', 'Completed'].map((s, i, arr) => {
                ...
                return (
                  <div key={s} className="flex items-center flex-shrink-0">
                    ...
                    <span className={`ml-1 text-[10px] ${isCurrent ? 'text-[#0C831F] font-bold' : 'text-gray-500'}`}>{ts(statusTranslationKeys[s])}</span>
        ...
        <Card className="mb-4">
          <CardContent className="p-4">
            <h3 className="font-bold text-gray-900 mb-3">{t('orderDetail.customerDetails')}</h3>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div className="flex items-center gap-2"><User className="h-4 w-4 text-gray-400" /><div><p className="text-gray-500 text-xs">{t('orderDetail.name')}</p><p className="font-medium">{order.customerName}</p></div></div>
              <div className="flex items-center gap-2"><Phone className="h-4 w-4 text-gray-400" /><div><p className="text-gray-500 text-xs">{t('orderDetail.mobile')}</p><p className="font-medium">{order.mobile}</p></div></div>
              {order.businessName && <div className="flex items-center gap-2 col-span-2"><Building className="h-4 w-4 text-gray-400" /><div><p className="text-gray-500 text-xs">{t('orderDetail.business')}</p><p className="font-medium">{order.businessName}</p></div></div>}
              {order.pickupDate && <div className="flex items-center gap-2 col-span-2"><MapPin className="h-4 w-4 text-gray-400" /><div><p className="text-gray-500 text-xs">{t('orderDetail.pickupDate')}</p><p className="font-medium">{order.pickupDate}</p></div></div>}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="mb-4">
          <CardContent className="p-4">
            <h3 className="font-bold text-gray-900 mb-3">{t('orderDetail.orderItems')}</h3>
        ...
        {order.specialNote && (
          <Card className="mb-4">
            <CardContent className="p-4">
              <h3 className="font-bold text-gray-900 mb-2">{t('orderDetail.specialNote')}</h3>
        ...
        <div className="flex flex-wrap gap-3 pb-8">
          ...
          <Button variant="outline" className="text-green-600 border-green-300" onClick={sendWhatsApp}>
            <MessageCircle className="h-4 w-4 mr-2" /> {t('orderDetail.sendWhatsApp')}
          </Button>
          <Button variant="outline" onClick={() => {
            ...
            <Printer className="h-4 w-4 mr-2" /> {t('orderDetail.printInvoice')}
          </Button>
        </div>
      </div>
    </div>
