  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#111827] dark:bg-[#111827]">
      <div className="max-w-3xl mx-auto px-4 py-4">
        <Button variant="ghost" className="mb-4 text-gray-600 dark:text-gray-400" onClick={() => navigate('admin-orders')}>
        ...
        {showAlert && alertData && (
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowAlert(false)}>
            <div className="bg-white dark:bg-gray-800 dark:bg-gray-800 rounded-2xl p-6 max-w-sm w-full shadow-xl" onClick={e => e.stopPropagation()}>
              <div className="text-center">
                ...
                <div className="bg-red-50 dark:bg-red-900/20 rounded-xl p-3 mt-4 text-sm">
                ...
                <Button className="mt-4 w-full bg-red-50 dark:bg-red-900/200 hover:bg-red-600 text-white" onClick={() => { setShowAlert(false); setEditingPayment(false); }}>
                  Samajh gaya
                </Button>
              </div>
            </div>
        ...
        <div className="bg-gradient-to-r from-[#0C831F] to-[#0fa828] rounded-2xl p-5 text-white mb-4">
          <div className="flex items-start justify-between">
            ...
            <div className="text-right">
              {editingPayment ? (
                <div className="flex items-center gap-1">
                  ...
                  <Input
                    ...
                    className="w-28 h-9 text-lg font-bold text-gray-900 dark:text-gray-100 bg-white/90 border-0"
                    autoFocus
                    step="1"
                    min="0"
                  />
              ...
              <Badge className="mt-2 bg-white/20 dark:bg-white/10 text-white hover:bg-white/30">{order.paymentMethod}</Badge>
            </div>
          </div>
              ...
              {['Pending', 'Accepted', 'Packing', 'Ready for Pickup', 'Completed'].map((s, i, arr) => {
                ...
                return (
                  <div key={s} className="flex items-center flex-shrink-0">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${isCurrent ? 'bg-[#0C831F] text-white' : isActive ? 'bg-green-200 text-green-800' : 'bg-gray-100 dark:bg-gray-700 text-gray-400'}`}>
                      {isActive && s !== order.orderStatus ? '✓' : i + 1}
                    </div>
                    <span className={`ml-1 text-[10px] ${isCurrent ? 'text-[#0C831F] font-bold' : 'text-gray-500 dark:text-gray-400'}`}>{s}</span>
                    {i < arr.length - 1 && <div className={`w-4 h-0.5 mx-1 ${isActive ? 'bg-green-300' : 'bg-gray-200 dark:bg-gray-700'}`} />}
                  </div>
                );
              })}
            </div>
        ...
        <Card className="mb-4">
          <CardContent className="p-4">
            <h3 className="font-bold text-gray-900 dark:text-gray-100 mb-3">Customer Details</h3>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div className="flex items-center gap-2"><User className="h-4 w-4 text-gray-400" /><div><p className="text-gray-500 dark:text-gray-400 text-xs">Name</p><p className="font-medium">{order.customerName}</p></div></div>
              <div className="flex items-center gap-2"><Phone className="h-4 w-4 text-gray-400" /><div><p className="text-gray-500 dark:text-gray-400 text-xs">Mobile</p><p className="font-medium">{order.mobile}</p></div></div>
              {order.businessName && <div className="flex items-center gap-2 col-span-2"><Building className="h-4 w-4 text-gray-400" /><div><p className="text-gray-500 dark:text-gray-400 text-xs">Business</p><p className="font-medium">{order.businessName}</p></div></div>}
              {order.pickupDate && <div className="flex items-center gap-2 col-span-2"><MapPin className="h-4 w-4 text-gray-400" /><div><p className="text-gray-500 dark:text-gray-400 text-xs">Pickup Date</p><p className="font-medium">{order.pickupDate}</p></div></div>}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="mb-4">
          <CardContent className="p-4">
            <h3 className="font-bold text-gray-900 dark:text-gray-100 mb-3">Order Items</h3>
            ...
            <div className="border-t mt-3 pt-3 space-y-1">
              {hasBargain && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 dark:text-gray-400 line-through">Original Total</span>
        ...
        {order.specialNote && (
          <Card className="mb-4">
            <CardContent className="p-4">
              <h3 className="font-bold text-gray-900 dark:text-gray-100 mb-2">Special Note</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">{order.specialNote}</p>
            </CardContent>
          </Card>
        )}
