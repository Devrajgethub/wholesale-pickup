...
import { useNavStore, useCartStore } from '@/lib/store';
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-[#111827] dark:bg-[#111827] flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">Add items to get started</p>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#111827] dark:bg-[#111827]">
      <div className="max-w-3xl mx-auto px-4 py-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">My Cart ({items.length})</h2>
          <Button variant="ghost" size="sm" className="text-red-500 text-xs" onClick={clearCart}>Clear All</Button>
        </div>
          ...
          <AnimatePresence>
            {items.map((item) => (
              <motion.div
                ...
                className="bg-white dark:bg-gray-800 dark:bg-gray-800 rounded-xl p-4 shadow-sm"
              >
                <div className="flex gap-3">
                  <div className="w-16 h-16 bg-gray-50 dark:bg-[#111827] rounded-lg flex items-center justify-center flex-shrink-0">
                    <span className="text-2xl">📦</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{item.productName}</h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{item.unit}</p>
                    <div className="flex items-baseline gap-2 mt-1">
                      <span className="font-bold text-gray-900 dark:text-gray-100">₹{item.price.toLocaleString()}</span>
                      {item.mrp > item.price && (
                        <span className="text-xs text-gray-400 line-through">₹{item.mrp.toLocaleString()}</span>
                      )}
                    </div>
                  </div>
                  <Button variant="ghost" size="icon" className="text-gray-300 dark:text-gray-600 hover:text-red-500 h-8 w-8 flex-shrink-0" onClick={() => removeItem(item.productId)}>
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t">
                  ...
                  <span className="font-bold text-gray-900 dark:text-gray-100">₹{(item.price * item.quantity).toLocaleString()}</span>
                </div>
              </motion.div>
            ))}
          </AnimatePresence>
        ...
        {/* Bill Summary */}
        <div className="bg-white dark:bg-gray-800 dark:bg-gray-800 rounded-xl p-5 mt-4 shadow-sm">
          <h3 className="font-bold text-gray-900 dark:text-gray-100 mb-3">Bill Details</h3>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-500 dark:text-gray-400">MRP Total</span>
